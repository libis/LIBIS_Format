  ...
  module Format
    module Converter
      ...
      class ImageConverter < Base
        ...
        def init(source)
          @source = source
          error "Cannot find image file '#{source}'." unless File.exist? source
        end
        ...
        def do_convert(target,format)
          ...
          command = 'format'
          command = 'watermarker.sh' if @wm_image
          ...
          if format == :JP2
            ...
            if result.match(/error/i)
              error "JPEG2000 conversion failed: #{result}"
            elsif result.match(/warning/i)
              warn "JPEG2000 conversion: #{result}"
            end
