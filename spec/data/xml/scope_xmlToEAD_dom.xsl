...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:variable name="XML_ARCHIVAL_TYPE" select="ARCHIEF/ARCHIVAL_TYPE"/>
		...
		<xsl:for-each select="ARCHIEF">
			<ead>
				<eadheader audience="external">
						...
						<xsl:if test="DATES_OF_DESCRIPTION">
							<xsl:for-each select="DATES_OF_DESCRIPTION">
								<publicationstmt>
									...
									<publisher>
										<xsl:value-of select="$XML_ARCHIEFINSTELLING_TITLE"/>, <xsl:value-of select="$XML_ARCHIEFINSTELLING_CITY"/>
									</publisher>
								</publicationstmt>
							</xsl:for-each>
						</xsl:if>
					...
					<profiledesc>
						<creation>Deze inventaris werd automatisch gegenereerd en gecodeerd.De EAD-export werd gecontroleerd en gevalideerd door de werkgroep archivarissen van LIAS K.U.Leuven (Leuvens Integraal Archiveringssysteem).<date>
								<xsl:value-of select="FILE_DATE"/>
							</date>
						</creation>
						<langusage>
				...
				<archdesc level="archief">
					<xsl:if test="$XML_ARCHIVAL_TYPE != ''">
						<xsl:attribute name="otherlevel">
							<xsl:value-of select="$XML_ARCHIVAL_TYPE"/>
						</xsl:attribute>
					</xsl:if>
					<descgrp>
						<head>1. IDENTIFICATIE</head>
						<repository label="Archiefinstelling">
							<corpname>
								<xsl:value-of select="$XML_ARCHIEFINSTELLING_TITLE"/>, <xsl:value-of select="$XML_ARCHIEFINSTELLING_CITY"/>
							</corpname>
						</repository>
						<unitid>
							...
							<xsl:attribute name="repositorycode">
								<xsl:value-of select="$XML_REFCODE"/>
							</xsl:attribute>
							...
							<p>
								<xsl:value-of select="$XML_SIGLUM"/>
							</p>
						</unitid>
						<unittitle>
							<head>1.2. Titel</head>
							<p>
								<xsl:value-of select="$XML_TITLE"/>
							</p>
						</unittitle>
					...
					<xsl:if test="((string-length(translate(normalize-space($XML_AUTH_URL) ,' ',''))!='0') or  (string-length(translate(normalize-space($XML_AUTH_NM) ,' ',''))!='0')) or ODIS_BIO or ARCHIVAL_HISTORY or ACQUISITION">
						<descgrp>
							<head>2. CONTEXT</head>
								...
								<origination label="Archiefvormer">
									...
									<extref>
										<xsl:attribute name="entityref">
											<xsl:value-of select="$XML_AUTH_DB"/>
										</xsl:attribute>
										<xsl:attribute name="show">new</xsl:attribute>
										<xsl:attribute name="href">
											<xsl:value-of select="$XML_AUTH_URL"/>
										</xsl:attribute>
					...
					<xsl:if test="((DESCRIPTION_OF_CONTENTS) or (GEOGRAPHICAL_INFORMATION) or (COLLECTION_TIME_PERIOD) or (RELATIVES_CORPORATIONS_FAMILIES_PEOPLE)) or VALUATION_AND_DESTRUCTION or ACCRUALS or ORDERING_AND_CLASSIFICATION">
						<descgrp>
					...
					<xsl:if test="ACCESS_REGULATIONS or REPRODUCTION_SPECIFICATIONS or LANGUAGE or PHYSICAL_PROPERTIES or FINDING_AID">
						<descgrp>
							...
							<xsl:if test="ABBREVIATIONS">
								<abbreviations>
									<head>4.6. Afkortingen</head>
					...
					<xsl:if test="ORIGINALS_EXISTENCE_LOCATION_OF_PRESERVATION or COPIES_EXISTENCE_STORAGE_AREA or UD_REFERENCES or RELATED_MATERIAL or PUBLICATIONS_ or AUDIOVISUAL_MATERIAL or PUBLICATIONS or BIBLIOGRAPHY or SOURCES">
						<descgrp>
							...
							<xsl:if test="COPIES_EXISTENCE_STORAGE_AREA">
								<altformavail>
									<head>5.2. Bestaan en bewaarplaats van kopieÃ«n</head>
							...
							<xsl:if test="UD_REFERENCES or RELATED_MATERIAL or AUDIOVISUAL_MATERIAL">
								<relatedmaterial label='non_pub'>
									<head>5.3. Verwante beschrijvingseenheden</head>
									<xsl:for-each select="UD_REFERENCES">
										<archref entityref="SCOPE" show="new">
											<xsl:attribute name="href">
												<xsl:value-of select="$XML_SCOPE_OPAC"/>
												<xsl:value-of select="REF_UD_ID"/>
											</xsl:attribute>
											<unittitle>
												<xsl:value-of select="REF_UD_TITLE"/>
											</unittitle>
											<note>
												<xsl:value-of select="REF_UD_ROLE"/>
											</note>
										</archref>
									</xsl:for-each>
							...
							<xsl:if test="PUBLICATIONS_">
								<relatedmaterial label='pub'>
									<head>5.4. Verwante publicaties</head>
								...
								<controlaccess>
									...
									<xsl:for-each select="CONTROL_ACCESS">
										<xsl:if test="CA_THSRS_NM='Persons'">
											<extref>
												<xsl:attribute name="href">
													<xsl:value-of select="CA_THSRS_URL"/>
												</xsl:attribute>
												<xsl:attribute name="show">new</xsl:attribute>
												<persname source="ODIS" role="subject">
													<xsl:value-of select="CA_THSRS_TREF_NM"/>
												</persname>
											</extref>
										</xsl:if>
									</xsl:for-each>
									...
									<xsl:for-each select="CONTROL_ACCESS">
										<xsl:if test="CA_THSRS_NM='Families'">
											<extref>
												...
												<xsl:attribute name="show">new</xsl:attribute>
												<famname source="ODIS" role="subject">
													<xsl:value-of select="CA_THSRS_TREF_NM"/>
												</famname>
											</extref>
										</xsl:if>
									...
									<xsl:for-each select="CONTROL_ACCESS">
										<xsl:if test="CA_THSRS_NM='Places'">
											<extref>
												...
												<xsl:attribute name="show">new</xsl:attribute>
												<geogname source="ODIS" role="subject">
													<xsl:value-of select="CA_THSRS_TREF_NM"/>
												</geogname>
											</extref>
										</xsl:if>
				...
				<did>
					...
					<xsl:for-each select="DGTL">
						<dao linktype="simple" href="http://imgs.ud.edu/archives/image/f12001_1.jpg" actuate="onrequest" show="new">
							<xsl:attribute name="href">
								<xsl:value-of select="DGTL_ACC_URL"/>
							</xsl:attribute>
							<xsl:attribute name="title">
								<xsl:value-of select="DGTL_DC_TITLE"/>
							</xsl:attribute>
						</dao>
					</xsl:for-each>
					...
					<xsl:if test="EXTENT or TYPE_OF_ARCHIVAL_MATERIAL or STAGE_OF_DEVELOPMENT">
						<physdesc>
							...
							<xsl:if test="TYPE_OF_ARCHIVAL_MATERIAL">
								<genreform label="MateriÃ«le vorm">
									<xsl:for-each select="TYPE_OF_ARCHIVAL_MATERIAL">
										<p>
											<xsl:value-of select="text()"/>
										</p>
					...
					<xsl:if test="COPIES_EXISTENCE_STORAGE_AREA">
						<altformavail label="Bestaan en bewaarplaatsen van kopieÃ«n">
							<xsl:for-each select="COPIES_EXISTENCE_STORAGE_AREA">
								<p>
									<xsl:value-of select="text()"/>
								</p>
						...
						<controlaccess>
									...
									<xsl:for-each select="CONTROL_ACCESS">
										<xsl:if test="CA_THSRS_NM='Persons'">
											<extref>
												...
												<xsl:attribute name="show">new</xsl:attribute>
												<persname source="ODIS" role="subject" label="Trefwoorden personen">
													<xsl:value-of select="CA_THSRS_TREF_NM"/>
												</persname>
											</extref>
										</xsl:if>
									...
									<xsl:for-each select="CONTROL_ACCESS">
										<xsl:if test="CA_THSRS_NM='Organizations'">
											<extref>
												...
												<xsl:attribute name="show">new</xsl:attribute>
												<corpname source="ODIS" role="subject" label="Trefwoorden organisaties">
													<xsl:value-of select="CA_THSRS_TREF_NM"/>
												</corpname>
											</extref>
										</xsl:if>
							...
							<xsl:if test="CONTROL_ACCESS/CA_THSRS_NM='Families'">
								<controlaccess>
									...
									<xsl:for-each select="CONTROL_ACCESS">
										<xsl:if test="CA_THSRS_NM='Families'">
											<famname source="ODIS" role="subject" label="Trefwoorden families">
												<xsl:value-of select="CA_THSRS_TREF_NM"/>
											</famname>
										</xsl:if>
									</xsl:for-each>
									...
									<xsl:for-each select="CONTROL_ACCESS">
										<xsl:if test="CA_THSRS_NM='Places'">
											<extref>
												...
												<xsl:attribute name="show">new</xsl:attribute>
												<geogname source="ODIS" role="subject" label="Geografische trefwoorden">
													<xsl:value-of select="CA_THSRS_TREF_NM"/>
												</geogname>
											</extref>
										</xsl:if>
								...
								<controlaccess>
									...
									<xsl:for-each select="CONTROL_ACCESS">
										<xsl:if test="CA_THSRS_NM='Subjects'">
											<subject source="ODIS" role="subject" label="Inhoudstrefwoorden">
												<xsl:value-of select="CA_THSRS_TREF_NM"/>
											</subject>
										</xsl:if>
									</xsl:for-each>
