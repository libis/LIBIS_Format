...
describe 'Identfier' do
  ...
  it 'should not attempt to identify a non-existing file or directory' do
    ...
    expect(result[:messages].all? do |x|
      x[0] == :error &&
        x[1] =~ /^Error running (Droid|Fido|File): IdentificationTool: file argument should be a path to an existing file or directory or a list of those/ # rubocop:disable Layout/LineLength
    end).to be_truthy
  end
